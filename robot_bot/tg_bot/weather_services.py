import requests
from .weather_codes import weather_code
...
class WeatherService:
    ...
    def get_current_weather_by_geo_data(lat, lon):
        ...
        if res.status_code != 200:
            raise WeatherServiceException('Cannot get geo data')
        cur_weather = res.json().get('current_weather')
        return f"{weather_code[cur_weather['weathercode']]}, Temperature: {cur_weather['temperature']} C, " \
               f"Wind speed: {cur_weather['windspeed']} km/h "
